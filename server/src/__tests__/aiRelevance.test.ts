...
实测在代码审查场景中，Sonnet 4.6 比 GPT-4 快 30%。`,
  ...
  {
    name: '直接相关：GitHub 开源 AI 工具库版本更新',
    keyword: 'GitHub 开源 AI 工具',
    content: `GitHub 开源社区近日发布了 AI 工具库的最新版本，收录了 100+ 热门开源 AI 项目。 
    该版本涵盖了从大模型推理、微调框架到自动化 Agent 开发等多个领域。 
    此次更新优化了检索算法，支持按编程语言和技术栈筛选工具，极大提升了开发者的选型效率。`,
    expectPass: true,
    expectKeywordMentioned: true,
    expectedRelevanceRange: [80, 100],
  },
...
建议初学者从免费工具开始体验，逐步过渡到付费方案。`,
  ...
  {
    name: '间接相关：其他 AI 导航网站，不是 GitHub',
    keyword: 'GitHub AI 导航',
...
describe('Query Expansion（查询扩展）', () => {
  ...
  it('中文关键词能正确扩展', async () => {
    const result = await expandKeyword('GitHub AI 导航');
    expect(result).toContain('GitHub AI 导航');
    expect(result.some(r => r.includes('GitHub'))).toBe(true);
  });
});
